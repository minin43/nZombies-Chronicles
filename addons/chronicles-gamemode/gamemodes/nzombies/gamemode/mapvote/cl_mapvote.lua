...
net.Receive("RAM_MapVoteStart", function()
    ...
    for i = 1, amt do
        ...
        nzMapVote.CurrentMaps[#nzMapVote.CurrentMaps + 1] = object
    end

    nzMapVote.EndTime = CurTime() + net.ReadUInt(32)

    if (IsValid(nzMapVote.Panel)) then nzMapVote.Panel:Remove() end

    nzMapVote.Panel = vgui.Create("RAM_VoteScreen")
    nzMapVote.Panel:SetMaps(nzMapVote.CurrentMaps)
end)
...
net.Receive("RAM_MapVoteUpdate", function()
    ...
    if (update_type == nzMapVote.UPDATE_VOTE) then
        ...
        if (IsValid(ply)) then
            ...
            if (IsValid(nzMapVote.Panel)) then
                nzMapVote.Panel:AddVoter(ply)
            end
        end
    elseif (update_type == nzMapVote.UPDATE_WIN) then
        if (IsValid(nzMapVote.Panel)) then
            nzMapVote.Panel:Flash(net.ReadUInt(32))
        end
    end
end)
...
function PANEL:Think()
    ...
    for k, v in pairs(self.Voters) do
        if (not IsValid(v.Player)) then
            ...
            if (not nzMapVote.Votes[v.Player:SteamID()]) then
                v:Remove()
            else
                local bar = self:GetMapButton(nzMapVote.Votes[v.Player:SteamID()])
    ...
    local timeLeft = math.Round(math.Clamp(nzMapVote.EndTime - CurTime(), 0,
                                           math.huge))
