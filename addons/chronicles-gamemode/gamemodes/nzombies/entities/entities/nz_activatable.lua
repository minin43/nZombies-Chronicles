...
function ENT:GetNZTargetText()
	...
	if self:IsElectricityNeeded() and not nzElec:IsOn() then
		...
		if self:GetCost() > 0 then
			return (!self:GetDisplayName() or #self:GetNZName() == 0) and "Press " .. nzDisplay.GetKeyFromCommand("+use") .. " to activate trap for " .. self:GetCost() .. " points." or "Press " .. nzDisplay.GetKeyFromCommand("+use") .. " to activate " .. self:GetNZName() .. " for " .. self:GetCost() .. " points."
		else
			return (!self:GetDisplayName() or #self:GetNZName() == 0) and "Press " .. nzDisplay.GetKeyFromCommand("+use") .. " to activate trap." or "Press " .. nzDisplay.GetKeyFromCommand("+use") .. " to activate " .. self:GetNZName() .. "."
		end
	end
end
