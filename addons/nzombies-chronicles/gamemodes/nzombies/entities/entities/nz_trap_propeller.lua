...
function ENT:SetupDataTables()
    ...
    self:NetworkVar( "Bool", 7, "ResetBladeOnStop", {KeyName = "nz_propeller_resetblade", Edit = {title = "Reset Blade on Stop?", order = 10, type = "Boolean"}} )
    self:SetResetBladeOnStop(false)
end
...
function ENT:Deactivation(creativeMode)
    ...
    if (self:GetResetBladeOnStop() and IsValid(self.BladeProp)) then
        self.BladeProp:ResetAngles()
    end
end
