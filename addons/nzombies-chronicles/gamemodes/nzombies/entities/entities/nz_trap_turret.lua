...
function ENT:SetupDataTables()
	...
	self:NetworkVar( "Int", 4, "FOVLimit", {KeyName = "nz_turret_fov_limit", Edit = {title="FOV Limit", order = 11, type = "Int", min = 1, max = 360}} )
	...
	self:NetworkVar("Bool", 8, "KillPlayers", {KeyName = "nz_turret_kill_players", Edit = {title="Kill Players?", order = 15, type = "Boolean"}})
	...
	if SERVER then
		if self.fNextFire < CurTime() then
			...
			local bullet = {
				...
				Tracer = 99999,
				Callback = function(ent, tr, dmginfo)
					local ent = tr.Entity 
					if (!ent:IsValidZombie()) then  -- We should only hurt zombies.
						if (!ent:IsPlayer()) then 
							dmginfo:ScaleDamage(0) 
							return
						elseif (!self:GetKillPlayers()) then
							dmginfo:ScaleDamage(0) 
							return
						end
					elseif (!ent.NZBoss) then
...
function ENT:GetEnemyVisible(ent)
	if SERVER then
		local livedLongEnough = (ent.GetLastSpawnTime == nil or ent:GetLastSpawnTime() == nil) and true or (CurTime() - ent:GetLastSpawnTime() > 2)
