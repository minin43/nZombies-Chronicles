...
function RTV.CanVote(ply)
    ...
    if !nzMapVote.AutoVoteUnlocked then
		return false, "You must reach round " .. nzMapVote.GetAutoUnlockRound() .. " before voting."
	end

    if nzRound:GetNumber() >= 1 or nzRound:GetNumber() == -1 then
        return false, "You must all die before the vote starts."
    end
