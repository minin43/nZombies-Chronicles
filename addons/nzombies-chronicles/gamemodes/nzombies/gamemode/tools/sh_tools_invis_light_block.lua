...
if SERVER then
	util.AddNetworkString("nz_InvisLightBlockCreation")
	
	net.Receive("nz_InvisLightBlockCreation", function(len, ply)
		if !ply:IsInCreative() then return end
		local vec1 = net.ReadVector()
		local vec2 = net.ReadVector()
		
		if net.ReadBool() then
			nzMapping:CreateInvisibleLightBlock(vec1, vec2, ply)
		else
...
nzTools:CreateTool("invis_light_block", {
	displayname = "Invisible Light Block Creator",
	...
	SecondaryAttack = function(wep, ply, tr, data)
		...
		for k,v in pairs(walls) do
			if v:GetClass() == "invis_light_block" then v:Remove() end
		end
	end,
	...
	displayname = "Invisible Light Wall Creator",
	...
	PrimaryAttack = function(wep, ply, tr, data)
		...
		elseif !height then
			...
			net.Start("nz_InvisLightBlockCreation")
