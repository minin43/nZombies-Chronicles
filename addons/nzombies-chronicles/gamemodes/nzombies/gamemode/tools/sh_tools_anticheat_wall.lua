...
if SERVER then
	util.AddNetworkString("nz_AntiCheatWallCreation")
	
	net.Receive("nz_AntiCheatWallCreation", function(len, ply)
		if !ply:IsInCreative() then return end
		local vec1 = net.ReadVector()
		local vec2 = net.ReadVector()
		
		if net.ReadBool() then
			nzMapping:CreateAntiCheatWall(vec1, vec2, ply)
		end
	end)
end

nzTools:CreateTool("anticheatwall", {
	displayname = "Anti-Cheat Wall",
	desc = "LMB: Set Corners, RMB: Remove AC Wall at spot, R: Reset corners. (Players can cheat in these)",
	...
	SecondaryAttack = function(wep, ply, tr, data)
		...
		for k,v in pairs(walls) do
			if v:GetClass() == "anticheat_wall" then v:Remove() end
		end
	end,
	...
	displayname = "Anti-Cheat Wall",
	desc = "Place where you don't want players to ever go.",
	...
	PrimaryAttack = function(wep, ply, tr, data)
		...
		elseif !height then
			...
			net.Start("nz_AntiCheatWallCreation")
