...
if (SERVER) then
    ...
    util.AddNetworkString("nzSounds.StopSoundFile")
...
function nzSounds:GetDefaultSound(event) -- Gets the default sound file from a sound event
    return nzSounds.Sounds.Default[event]
end

function nzSounds:PlayEnt(event, ent, noOverlap) -- Plays sound event on an entity (and must be close to actually hear it)
...
function nzSounds:StopFileCS(file, ply, ent) -- Stops a clientside sound file from playing
    if (file == nil || !isstring(file)) then return end

    if (SERVER) then
        net.Start("nzSounds.StopSoundFile")
        net.WriteString(file)
        net.WriteEntity(ent)
...
function nzSounds:Play(event, ply) -- Plays a sound event everywhere either for 1 or all players
    local snd = nzSounds:GetSound(event)
    if (snd == nil || !isstring(snd)) then return end
    ...
    if (CLIENT) then
        if ((event == "RoundStart" || event == "RoundEnd") and GetConVar("nz_round_sounds"):GetInt() < 1) then return end
        if (event == "GameEnd" and GetConVar("nz_gameover_music"):GetInt() < 1) then return end
...
if (CLIENT) then
    ...
    net.Receive("nzSounds.PlaySoundFile", function()
        local file = net.ReadString()
        local ent = net.ReadEntity()
        nzSounds:PlayFileCS(file, nil, ent)
    end)

    net.Receive("nzSounds.StopSoundFile", function()
        local file = net.ReadString()
        local ent = net.ReadEntity()
        nzSounds:StopFileCS(file, nil, ent)
    end)
