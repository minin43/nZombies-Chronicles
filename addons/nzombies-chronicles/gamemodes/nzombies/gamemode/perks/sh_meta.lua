...
if SERVER then
	function playerMeta:GivePerkBottle(id) -- Added by Ethorbit for handling perk bottles in a more nZombies consistent way
		local perk = nzPerks:Get(id)
		 if !perk then return print(id, "is not a valid perk ID.") end

		 local bottle_class = perk.bottle_ent
		 if !weapons.Get(bottle_class) then print("Perk ID:", id, "has no perk bottle weapon assigned, using the nZombies default...") end

		 if !self:GetNotDowned() then
		 	self:Give(bottle_class or "nz_perk_bottle")
		end
	end
