-- File created by Ethorbit for actual TFA support
-- since they couldn't be bothered to test
...
-- This may break with a future TFA update, but oh fucking well \o/ they made everything local
...
if CLIENT then
    ...
    hook.Add("HUDWeaponPickedUp", "NZ_Crosshair_Fix", function(wepom)
        if !wepom.IsTFAWeapon then return end

        wepom.DoDrawCrosshair = function() --  ^^^ For some reason this was the only way to successfully override the damn thing without weird crosshair movements
            local self = wepom
