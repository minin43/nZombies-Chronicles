	...
	hook.Add("EntityRemoved", "NZBossFix", function(ent)
		if (nzRound) then
			if IsValid(ent) and ent.NZBoss then
				...
				if (nzRound and nzRound:GetNextBossRound() and nzRound:GetNextBossRound() <= nzRound:GetNumber()) then 
					if (isstring(nzRound.BossType)) then
						nzRound:SpawnBoss(nzRound.BossType)
					end
				end
			end
		end
...
nzRound:AddBossType("Panzer", "nz_zombie_boss_panzer", {
	...
	initfunc = function()
		...
		nzRound:SetNextBossRound(math.random(10, 12))
	...
	spawnfunc = function(panzer)
		...
		panzer:SetHealth(hp)
		panzer:SetMaxHealth(hp)

		BroadcastLua("surface.PlaySound('nzr/effects/panzeralarm.mp3')")
	end,
...
nzRound:AddBossType("Antlion Guard", "nz_zombie_hl2_antlionguard", {
	specialspawn = true,
	initfunc = function()
		--nzRound:SetNextBossRound(math.random(6,8)) -- Randomly spawn in rounds 6-8
		nzRound:SetNextBossRound(math.random(10, 12))
	...
	spawnfunc = function(antlion_guard)
		local hp = 5000
		...
		antlion_guard:SetHealth(hp)
		antlion_guard:SetMaxHealth(hp)
	end,
	deathfunc = function(antlion_guard, killer, dmginfo, hitgroup)
		nzRound:SetNextBossRound(nzRound:GetNumber() + math.random(3,5)) -- Delay further boss spawning by 3-5 rounds after its death
		--nzRound:SetNextBossRound(nzRound:GetNumber() + 6)
